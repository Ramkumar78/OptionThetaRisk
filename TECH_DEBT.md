...
- **Priority**: High
- **Status**: Partially Addressed. `screen_turtle_setups` and `screen_trend_followers_isa` have been refactored to use dedicated strategy classes (`option_auditor/strategies/`).
...
- **Hardcoded Financial Constants**: Addressed by replacing hardcoded values (RISK_FREE_RATE, Indicator Lengths) in `screener.py` and strategies with imports from `common/constants.py` and `common/screener_utils.py`.
