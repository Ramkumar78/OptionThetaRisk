...
class TestStrategyClassification:
    ...
    def test_wheel_strategy(self):
        """
        Test the Wheel Strategy classification.
        Definition: A closed Short Put followed by an open Long Stock position.
        The stock quantity must match the put quantity * 100.
        Note: Wheel logic is in build_strategies, not _classify_strategy.
        """
        from option_auditor.strategy import build_strategies

        # We need to construct a DataFrame of legs to pass to build_strategies
        t1 = pd.Timestamp("2024-01-01 10:00")
        t2 = pd.Timestamp("2024-01-15 16:00")
        t3 = t2 + pd.Timedelta(hours=1) # Stock assigned shortly after put close/expiry

        legs = []
        # Short Put: Open (-1)
        legs.append({
            "datetime": t1, "contract_id": "P1", "symbol": "XYZ",
            "qty": -1, "proceeds": 100.0, "fees": 1.0,
            "strike": 100.0, "right": "P", "expiry": t2
        })
        # Short Put: Close (1) - e.g. Assignment or Buy Back
        legs.append({
            "datetime": t2, "contract_id": "P1", "symbol": "XYZ",
            "qty": 1, "proceeds": 0.0, "fees": 0.0,
            "strike": 100.0, "right": "P", "expiry": t2
        })
        # Stock: Open (100)
        legs.append({
            "datetime": t3, "contract_id": "STOCK", "symbol": "XYZ",
            "qty": 100, "proceeds": -10000.0, "fees": 5.0,
        ...
        strat = strategies[0]
        assert strat.strategy_name == "Wheel"
        assert strat.symbol == "XYZ"
