...
const Screener: React.FC<ScreenerProps> = () => {
  ...
  const [authMethod, setAuthMethod] = useState<'token' | 'legacy'>('token');
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-[28rem] border border-gray-200 dark:border-gray-700">
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="text-lg font-bold text-gray-900 dark:text-white">Tastytrade Setup</h3>
                        <div className="flex gap-2 text-xs">
                            <button
                                onClick={() => setAuthMethod('token')}
                                className={clsx("px-2 py-1 rounded", authMethod === 'token' ? "bg-primary-100 text-primary-700 dark:bg-primary-900 dark:text-primary-300" : "text-gray-500")}
                            >
                                API Key (New)
                            </button>
                            <button
                                onClick={() => setAuthMethod('legacy')}
                                className={clsx("px-2 py-1 rounded", authMethod === 'legacy' ? "bg-primary-100 text-primary-700 dark:bg-primary-900 dark:text-primary-300" : "text-gray-500")}
                            >
                                Legacy
                            </button>
                        </div>
                    ...
                    <p className="text-xs text-gray-500 mb-4 dark:text-gray-400">
                        {authMethod === 'token'
                            ? "Use your Refresh Token and Client Secret (Open API). Production Ready."
                            : "Use Username/Password (Sandbox/Legacy only). May not work with newer SDKs."}
                    </p>

                    {authMethod === 'token' ? (
                        <>
                            <div className="mb-3">
                                <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Refresh Token (Required)</label>
                                <input
                                    type="password"
                            ...
                            <div className="mb-3">
                                <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Client Secret (Required)</label>
                                <input
                                    type="password"
